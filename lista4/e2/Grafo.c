...
void imprime_Grafo(Grafo *gr)
{
    ...
    for(i=0; i < gr->nro_vertices; i++)
    {
        printf(".Vertice %s, vinculado a: ", cities[i]);
        for(j=0; j < gr->grau[i]; j++)
        {
            if(gr->eh_ponderado)
            {
                city = gr->arestas[i][j];
                printf("%s(Peso %.2f), ", cities[city], gr->pesos[i][j]);
            }
            else
            {
                printf("%s - %d, ", cities[j], gr->arestas[i][j]);
            }
        }
        printf("\n");
    }
...
/**
 * Encontra a aresta de menor peso que conecta dois componentes diferentes
 * Percorre todas as arestas do grafo para encontrar a aresta com
 * o menor peso que conecta dois vértices de componentes diferentes.
 * Se encontrar, a função atualiza os vértices de origem e destino
 * e retorna 1. Se nenhuma aresta desse tipo existir, retorna 0.
 */
int findMinEdge(int NV, Grafo *gr, int *arv, int *orig, int *dest) {
    float menorPeso = -1.0;
    int encontrou = 0;
    ...
    for (int i = 0; i < NV; i++) {
        for (int j = 0; j < gr->grau[i]; j++) {
            if (arv[i] != arv[gr->arestas[i][j]]) {
                if (menorPeso == -1 || gr->pesos[i][j] < menorPeso) {
...
void printResult(int *pai, int NV) {
    for (int i = 0; i < NV; i++) {
        if (pai[i] != -1) {
            printf("%s: + %s\n", cities[i], cities[pai[i]]);
        }
    }
}
...
void kruskal(Grafo *gr, int orig, int *pai) {
    ...
    while (1) {
        int dest;
        if (!findMinEdge(NV, gr, arv, &orig, &dest)) {
            break;
        }
